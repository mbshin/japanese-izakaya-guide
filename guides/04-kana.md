...
>
> 관련: [00-say.md](../field/say.md) · [03-conversation.md](./03-conversation.md) · [90-youtube.md](../resources/youtube.md)
...
| **가타카나** カタカナ | ア イ ウ | 외래어, 외국 이름, 강조. **두 번째** |
| **한자** 漢字 | 出口 生<br>でぐち なま | 명사·동사 어간. 한국인은 뜻으로 유추 가능 |

> 💡 여행·회화 목적이면 **히라가나 + 가타카나 + 한자 "뜻만" 알기**로 충분합니다.
> 한자 읽는 법(음독·훈독)까지 외우는 건 훨씬 나중 일입니다.
...
**46자를 두 번 외우는 게 아니라, 46개 소리에 글자 두 벌이 붙는 것**입니다.
...
```text
さくら  sa-ku-ra  (3박자)
とうきょう  to-u-kyo-u  (4박자)  ← 도쿄가 4박자입니다
```
...
| 글자 | 설명 |
|---|---|
| **を** | 소리는 `o`. **목적격 조사로만** 씁니다. 단어 안에는 안 나옵니다 |
| **ん** | 유일한 받침 소리. 뒤 글자에 따라 `ㄴ/ㅁ/ㅇ`으로 변합니다 |

```text
ほん hon (책)        → ㄴ
さんぽ sanpo (산책)  → ㅁ  (실제로는 sampo에 가깝게)
りんご ringo (사과)  → ㅇ
```
...
| 짝 | 구분법 |
|---|---|
| **あ / お** | あ는 가로획이 하나, お는 오른쪽 위에 점 |
| **ぬ / め** | ぬ는 끝에 고리가 있음, め는 없음 |
| **は / ほ** | ほ는 가로획이 하나 더 |
| **る / ろ** | る는 끝이 고리로 말림, ろ는 열림 |
| **さ / き** | き는 가로획 2개 |
| **れ / わ / ね** | れ 직선, わ 둥근 고리, ね 고리+꼬리 |
| **い / り** | い는 두 획이 분리, り는 이어짐 |
| **つ / し** | つ는 가로 방향, し는 세로 방향 |
...
> 💡 `ぢ`·`づ`는 거의 안 씁니다. `じ`·`ず`로 대체됩니다. 신경 쓰지 마세요.
> ⚠️ **한국인 최대 난관**: `ざ(za)`를 `じゃ(ja)`로 발음하는 것. 그리고 단어 첫머리의 `が·ざ·だ·ば`를 무성음으로 내는 것.
...
> ⚠️ **큰 글자 vs 작은 글자**는 완전히 다릅니다.
> `きや ki-ya` (2박자) ≠ `きゃ kya` (1박자)
...
```text
きって   ki(멈춤)te   kitte    우표
がっこう  ga(멈춤)kou  gakkou   학교
いっぱい  i(멈춤)pai   ippai    한 잔
ざっし   za(멈춤)shi  zasshi   잡지
```

> 한국어 받침 `ㅅ`처럼 끊어 준다고 생각하면 가깝습니다. 「깃테」「각코-」
> ⚠️ `つ` (큰 것, tsu 소리) 와 `っ` (작은 것, 멈춤)는 다른 글자입니다.
...
う단 + う    すうがく    suugaku   수학
え단 + い    せんせい    sensei    선생님  ← え+い 인데 「세-」로 읽음
お단 + う    とうきょう  toukyou   도쿄    ← お+う 인데 「토-」로 읽음
```

> ⚠️ `えい`는 「에이」가 아니라 **「에-」**, `おう`는 「오우」가 아니라 **「오-」** 입니다.
> `せんせい` = 셍세**-** (셍세이 ❌)
...
탁음·반탁음·요음도 히라가나와 규칙이 같습니다.
`ガギグゲゴ` `ザジズゼゾ` `ダヂヅデド` `バビブベボ` `パピプペポ` / `キャ キュ キョ` …
...
| 짝 | 구분법 |
|---|---|
| **シ shi / ツ tsu** | シ는 점이 **가로**로 눕고 아래에서 위로, ツ는 점이 **세로**로 서고 위에서 아래로 |
| **ソ so / ン n** | ソ는 위에서 아래로 짧게, ン은 아래에서 위로 |
| **ク ku / タ ta / ワ wa** | ク 2획, タ 3획, ワ는 각진 고리 |
| **ス su / ヌ nu** | ヌ는 가로획을 가로지름 |
...
| 표기 | 소리 | 예 |
|---|---|---|
| ファ フィ フェ フォ | fa fi fe fo | ファミマ (패밀리마트) |
| ティ ディ | ti di | パーティー (파티) |
| ウィ ウェ ウォ | wi we wo | ウィスキー (위스키) |
| シェ チェ ジェ | she che je | シェア (셰어) |
| ヴ | v | ヴィーガン (비건) |
...
> ✍️ **1주차 과제**: 본인 이름의 가타카나 표기를 확정하고, 종이에 쓸 수 있게 연습하세요.
> 자기소개할 때 냅킨에 써서 보여주면 상대가 훨씬 잘 기억합니다.
...
3일차  ま행~ん (16자)        + 전체 랜덤 테스트
4일차  탁음·반탁음 (25자)     + 요음
5일차  촉음·장음 규칙        + 실제 간판/메뉴 읽어보기
6일차  가타카나 ア행~ナ행
7일차  가타카나 나머지 + 내 이름 표기 확정
```
...
| 도구 | 용도 |
|---|---|
| **Anki** | 히라가나/가타카나 덱. 하루 10분 간격 반복 |
| **Renshuu / Kana (앱)** | 게임식 반복 |
| **Duolingo** | 초반 동기부여용 |

**② 단어로 익히기 — 글자 낱개는 잘 안 붙습니다**
...
일본 편의점 상품 사진, 라멘집 메뉴판, 역 간판을 검색해서 **소리 내어 읽어보세요.**
읽히기 시작하면 급격히 재밌어집니다.
...
> 🎬 **연습 영상**: [히라가나 받아쓰기 연습](https://www.youtube.com/results?search_query=%ED%9E%88%EB%9D%BC%EA%B0%80%EB%82%98%20%EB%B0%9B%EC%95%84%EC%93%B0%EA%B8%B0%20%EC%97%B0%EC%8A%B5) · [가타카나 외래어 읽기](https://www.youtube.com/results?search_query=%EA%B0%80%ED%83%80%EC%B9%B4%EB%82%98%20%EC%99%B8%EB%9E%98%EC%96%B4%20%EC%9D%BD%EA%B8%B0) · [일본어 단어 읽기 연습 초급](https://www.youtube.com/results?search_query=%EC%9D%BC%EB%B3%B8%EC%96%B4%20%EB%8B%A8%EC%96%B4%20%EC%9D%BD%EA%B8%B0%20%EC%97%B0%EC%8A%B5%20%EC%B4%88%EA%B8%89)
...
작은 っ  →  자음을 두 번   kitte → きって
작은 ゃ  →  x 또는 l 접두   xya / lya
ん       →  nn 또는 n'

변환 후 스페이스바 → 한자/가타카나 후보 선택
```

> 💡 **로마자 입력이 압도적으로 편합니다.** 가나 자판(フリック)은 나중에 배워도 됩니다.
...
현장에서 쓸 문장은 [00-say.md](../field/say.md) 에 정리돼 있습니다.
